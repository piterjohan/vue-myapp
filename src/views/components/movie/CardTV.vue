<script setup>
defineProps(['tvs', 'tvGenres']);
</script>

<template>
  <div class="carousel__item">
  
    <div class="position-relative card" style="width: 18rem" v-if="tvs.poster_path">
      <img
        :src="'https://image.tmdb.org/t/p/w500' + tvs.poster_path"
        class="card-img-top"
        :alt="tvs.title + ' ' + '(' + tvs.original_title + ')'"
      />

      <!-- genre -->
      <div class="position-absolute genre-list">
        <template v-for="(genre, indexGenre) in tvs.genre_ids" :key="indexGenre">
          <span class="badge">
            {{ tvGenres.genres.find((genreList) => genreList.id === genre)?.name }}
          </span>
        </template>
      </div>
      ...
      <div class="position-absolute rating">
        <span> {{ Math.round( (tvs.vote_average * 100) / 10) }}% </span>
      </div>

      <!-- button -->
      <div class="position-absolute btn-wrapper">
        <router-link :to="{ name: 'tv_detail', params: { id: tvs.id } }">
